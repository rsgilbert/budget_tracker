...
class MyApp extends StatelessWidget {
  ...
  static final _router = GoRouter(routes: [
    ...
    GoRoute(path: '/manage-budget-entry', name: 'manage', builder: (context, state) => ManageBudgetEntryScreen(budgetEntry: state.extra as BudgetEntry?))
  ]);
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _refreshBudgetEntries() async {
    try {
      ...
      final budgetEntries = response.data?.items;
      if(response.hasErrors) {
        safePrint('Errors: ${response.errors}');
        return;
      }
      setState(() {
        _budgetEntries = budgetEntries!.whereType<BudgetEntry>().toList();
      });
...
class _ManageBudgetEntryScreenState extends State<ManageBudgetEntryScreen> {
  ...
  Future<void> submitForm() async {
    ...
    if(_isCreate) {
      ...
      final newEntry = BudgetEntry(title: title, amount: amount, description: description.isNotEmpty ? description : null);
      final request = ModelMutations.create(newEntry);
      final response = await Amplify.API.mutate(request: request).response;
      safePrint('Create result: $response');
    }else {
      // update budget entry instead 
      final updateBudgetEntry = _budgetEntry!.copyWith(title: title, description: description.isNotEmpty ? description : null, amount: amount);
      final request = ModelMutations.update(updateBudgetEntry);
      final response = await Amplify.API.mutate(request: request).response;
      safePrint('Update result: $response');
    }
    // navigate back to home page after create/update executes
    if(mounted) {
      context.pop();
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      child: SingleChildScrollView(child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        TextFormField(controller: _titleController, decoration: const InputDecoration(labelText: 'Title (required)'),validator: (value) {
        ...
        TextFormField(controller: _amountController, keyboardType: const TextInputType.numberWithOptions(signed: false, decimal: true), decoration: const InputDecoration(labelText: 'Amount (required)'), 
          validator: (value) {
            if(value == null || value.isEmpty) {
...
ElevatedButton(onPressed: submitForm, child: Text(_titleText))
      ]))))
      ),
      )
    );
